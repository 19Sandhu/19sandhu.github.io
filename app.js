...
function clearThePage() {                 //Clear the web page
    document.querySelector("#results").innerHTML = "";
}

function populateRow(videos = videosList) {            //Simple All video click
    clearThePage();
    for (let i = 0; i < videos.length; i++) {
        document.querySelector("#results").innerHTML += generateRowHTML(videos[i]);
    }
}

function searchVideo(str) {           //Seaching Something on Website
    let keyword = str.toLowerCase();
    let matchedVideos = [];
    for (let i = 0; i < videosList.length; i++) {
        if (videosList[i].title.toLowerCase().includes(keyword) || videosList[i].description.toLowerCase().includes(keyword) || videosList[i].author.toLowerCase().includes(keyword)) {
            matchedVideos.push(videosList[i]);
        }
    }
    if (matchedVideos.length == 0) {
        clearThePage();
        document.querySelector("#results").innerHTML = `<p>No videos found with the keyword ${str}</p>`;
        setTimeout(clearThePage, 3000);
    }
    else {
        populateRow(matchedVideos);
    }
}

function getValue() {
    var srch = document.querySelector("#txt-search");
    var search_value = srch.value;
    if (search_value != "") {
        searchVideo(search_value); //Getting something from search box
        srch.value = "";
    }
    else {
...
document.querySelector("#txt-search").addEventListener("keyup", function (event) {
    if (event.keyCode == 13) {
        document.getElementById("btn-search").click(); //Enter Keyword
    }
})

document.querySelector("#btn-search").addEventListener('click', getValue);
document.querySelector("#btn-show-all").addEventListener('click', function () { populateRow(); });